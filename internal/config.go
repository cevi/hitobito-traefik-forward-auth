...
type Config struct {
	...
	DefaultProvider      string               `long:"default-provider" env:"DEFAULT_PROVIDER" default:"google" choice:"google" choice:"oidc" choice:"generic-oauth" choice:"hitobito-oauth" description:"Default provider"`
	SkipMailVerification bool                 `long:"skip-mail-verification" env:"SKIP_MAIL_VERIFICATION" description:"Skip the verification step of the email (should be used together with group-id-verification of hitobito)"`
	GroupIDs             CommaSeparatedList   `long:"group-ids" env:"GROUP_IDS" env-delim:"," description:"List of valid group ids. Only users that are part of such a group are permitted."`
...
func (c *Config) GetProvider(name string) (provider.Provider, error) {
	...
	case "hitobito-oauth":
		return &c.Providers.Hitobito, nil
	}
